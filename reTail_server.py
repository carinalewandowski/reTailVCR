    ...
    if request.method == 'POST':
        ...
        if (request.form.get('deletebid') is not None):
            ...
            if (max_bid_user != None and str(current_max_bid[2]) == price):
                ...
                errormsg = "The bid you were trying to accept was removed by the bidder right before you hit accept, so we could not process the transaction. Sorry for the inconvenience."
                html = render_template('track.html', errormsg=errormsg)
                response = make_response(html)
                return response

    if request.args.get('action') == None:
        print("first arrivalllllllllll")
        html = render_template('track.html', errormsg='')
        response = make_response(html)
        return response
    else:
